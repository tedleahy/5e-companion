...
Custom subclasses are the main user-owned reference type today. Active custom subclass lookups filter `archivedAt: null`, and manager + inline level-up creation paths treat duplicate names as case-insensitive per user/class. Every subclass owns a required 1–20 `selectionLevel`; allocation validation permits no subclass at any class level but rejects a newly assigned subclass below that row's level. `saveCharacterSheet` grandfathers an unchanged assignment if a custom subclass's level was raised later. The manager also reconciles reusable custom subclass feature definitions transactionally as `Feature` rows with `kind: SUBCLASS_FEATURE`, scoped by `ownerUserId`, `classId`, and `subclassId`; omitted submitted feature ids are removed. Archiving a custom subclass hides it from future create and level-up selections, but existing characters keep their subclass relation; `saveCharacterSheet` may preserve an archived subclass id only when it was already attached to that same character.
