...
2. **Shell extension — done.** All three slots are optional, so the eight browse-only callers are unchanged. Row actions moved outside the row's `Pressable` rather than nesting inside it, so tapping edit or delete cannot also open the detail behind it; with no actions declared the main area still fills the row, so the pressed highlight is unchanged for existing callers.
3. **Screen rewrite — done.** `components/compendium/subclass-compendium.tsx` browses through `useCompendiumBrowse` and the extended shell; the route is a five-line wrapper like the other categories. Writes live in `use-custom-subclass-manager.ts`, and the sheet, dialog, and snackbar in `subclass-manager-overlay.tsx`, so browsing and writing stay separable. `SubclassManagerCard`, `SubclassListRow`, and `SubclassClassFilterChips` are deleted, along with the `SubclassManagerRow` type; the form sheet, feature card, and draft helpers are unchanged. The session guard, the three full-screen returns, and the client-side join are gone.

4. **Tests — done.** The suite moved to `components/compendium/__tests__/subclass-compendium.test.tsx`, matching the other categories. Playwright helpers and specs were re-pointed at the shell's testIDs (`compendium-collection-list`, `compendium-row-*`, `compendium-detail-back`, `subclass-class-filter-*`); custom rows have no `srdIndex`, so `compendium-row-{value}` still yields the row id the CRUD helpers need.

Two things the migration surfaced:

- The shell pluralised with `${noun}s`, which read as "subclasss" in the search placeholder, the SRD switch, the result count, and the detail back label. `pluralNoun` in `compendium-browse-presentation.ts` now handles sibilant endings, and every site uses it.
- `useCompendiumBrowse` now also returns the raw query `error`, so a screen can branch on the failure. Subclasses needs it for the unauthenticated redirect; `collection.error` only carries a message.
