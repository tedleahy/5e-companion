...
- **Custom subclasses and custom features** are first-class here — the wizard can produce a `customSubclass` or custom features that then round-trip via `draftApplication.ts` → `saveCharacterSheet`. The subclass selection step also includes active custom subclasses and reusable custom subclass feature definitions created in the `/compendium/subclasses` manager for the matching parent class.
...
- **Class options are authenticated reference data** — the picker consumes `availableClasses`, including active owned custom classes. Hit dice, custom multiclass prerequisites, and configured multiclass proficiency gains come from that definition; SRD-only advanced mechanics remain keyed by nullable SRD identifiers.
- **Hit-point presentation uses the selected class hit die** — labels, averages, and accessibility text read `classDefinition.hitDie` (falling back to SRD maps only when no definition is present).
- **Local draft preview keeps custom spellcasting** — `applyLevelUpToDraft` threads the selected class's resolved definition into spell-slot and profile derivation so confirming a level-up does not briefly wipe custom slots before the server response.
- **Multiclass proficiency choices are typed pick-N groups** — `getMulticlassProficiencyGains` surfaces fixed armor/weapon/tool grants plus every independently limited choice group (skills and non-skills). Wizard state stores `selections: { choiceGroup, values }[]` using proficiency identities (`srdIndex ?? id`, including `skill-*` for skills). Confirming level-up writes those into `CharacterSheetDraft.pendingProficiencyChoices` (same shape as create/save `ProficiencyChoiceSelectionInput`) and also merges labels into local skill/trait draft state for immediate UI. `mapCharacterSheetDraftToSaveInput` submits them as `SaveCharacterSheetInput.proficiencyChoices`. On save, the server detects newly added class rows, derives each class's MULTICLASS choice groups, validates exact unique picks/membership, and merges fixed grants plus validated choices into persisted skills/traits. Ordinary manual trait/skill edits without a newly added class do not require provenance. Pending selections clear with draft reset / successful save; changing the selected class in the wizard clears stale wizard selections. Bard's musical-instrument MULTICLASS choice is a real picker, not a placeholder tool label.
- **Custom progression is normalized** — standard/pact slots, ASI grants, features, and subclass options come from the class's level rows. Unique resources such as Rage and Sorcery Points remain the existing SRD-specific feature/resource handling rather than a universal resource engine.
