...
- Browse-only categories share a controlled `CompendiumCollection` shell for search, the Include-SRD switch, optional category filters, result totals, loading/error/empty states, virtualized rows, and in-place detail. Category filters reset through an explicit callback. A matching loaded `?value=` opens detail once on arrival; invalid or inaccessible values leave the list visible, and using the detail back control does not immediately reopen the unchanged deep link. List/detail transitions respect the system reduced-motion preference.
- `/compendium/languages` adds exclusive Standard/Exotic and script filters, script cartouche marks, speaker and learning-source details, usage counts, and same-script peer jumps. Peers require a recorded script — languages with missing or blank script metadata are not grouped together. `/compendium/feats` presents prerequisite thresholds and parses display-only benefit bullets from the existing description. `/compendium/races` exposes lineage bonuses, traits, languages, usage counts, in-page jump links to Traits, Languages, Life & build, and Subraces, and stable-value subrace navigation. `/compendium/subraces` shows a parent → subrace inheritance diagram, added traits, inherited parent rules, usage counts, and a jump back to the parent race. `/compendium/backgrounds` presents grants, reference-only starting equipment, the defining feature, and collapsible suggested characteristics. These screens are browse-only and expose no custom-content actions.
- Language usage counts are best-effort: creation currently persists racial language grants through the race relation, but does not write background language choices to `CharacterLanguage`; future join rows are included when present.
