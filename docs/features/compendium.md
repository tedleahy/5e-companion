...
- `/compendium/languages` adds exclusive Standard/Exotic and script filters, script cartouche marks, speaker and learning-source details, usage counts, and same-script peer jumps. `/compendium/feats` presents prerequisite thresholds and parses display-only benefit bullets from the existing description. `/compendium/races` and `/compendium/subraces` expose lineage bonuses, traits, languages, inherited parent rules, usage counts, and stable-value cross-navigation. `/compendium/backgrounds` presents grants, reference-only starting equipment, the defining feature, and collapsible suggested characteristics. These screens are browse-only and expose no custom-content actions.
