...
- **Paper `Portal` + controlled `TextInput`** can duplicate characters on Android when parent state outside the portal re-renders each keystroke. Use `FantasyFormTextInput` for form fields; custom-styled inputs in `BottomSheetShell` must buffer the controlled value locally and disable autocorrect.
